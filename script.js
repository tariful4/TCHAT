...
// Pure Email Authentications Logic
const login = async () => {
    const email = document.getElementById('loginEmail').value.trim();
    const ps = document.getElementById('loginPass').value.trim();
    if(!email || !ps) { alert("Please fill all fields!"); return; }
    
    toggleLoader(true);
    try {
        const userCredential = await signInWithEmailAndPassword(auth, email, ps);
        ...
        if(userDoc.exists()) {
            ...
            const fallbackUser = { id: uid, email: email, username: email.split('@')[0], profilePic: defaultPic };
            localStorage.setItem('user', JSON.stringify(fallbackUser));
            location.reload();
        }
    } catch(e) { 
        toggleLoader(false); 
        alert("Invalid login data: " + e.message); 
    }
}
...
const register = async () => {
    ...
    toggleLoader(true);
    try {
        const userCredential = await createUserWithEmailAndPassword(auth, email, ps);
        const id = userCredential.user.uid;
        
        const user = { id, email: email, username: email.split('@')[0], password: ps, profilePic: defaultPic };
    ...
    if(!isMe) {
        ...
        currentVisitorsListener = onSnapshot(visitorsQuery, (vSnap) => {
            ...
            vSnap.forEach(child => {
                ...
                listContainer.innerHTML += `<div class="visitor-item"><img src="${v.pic || defaultPic}" class="visitor-img"><div class="visitor-info"><span class="visitor-name" data-uid="${v.uid}">${v.name}</span><div class="visitor-time">Visited: ${new Date(v.timestamp).toLocaleString()}</div></div></div>`;
            });
...
const toggleLike = async (pid) => {
    ...
    if (isAlreadyLiked) { likeBtn.classList.remove('liked'); likeCountEl.innerText = currentCount - 1; } 
    else { likeBtn.classList.add('liked'); likeCountEl.innerText = currentCount + 1; }
    ...
    onSnapshot(qRef, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
            ...
            if (change.type === "added") {
                if (!existingCard) {
                    const wrapper = document.createElement('div'); wrapper.innerHTML = generatePostHTML(p);
            ...
            } else if (change.type === "removed" && existingCard) { existingCard.remove(); }
        });
...
window.addEventListener('popstate', (event) => {
    ...
    if (!inboxPage.classList.contains('hidden')) {
        ...
        if (confirm("Do you want to exit TCHAT?")) { navigator.app ? navigator.app.exitApp() : window.close(); } 
